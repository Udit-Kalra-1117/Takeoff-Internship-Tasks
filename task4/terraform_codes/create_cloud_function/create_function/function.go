...
func createEMP(w http.ResponseWriter, r *http.Request) {
	...
	query := client.Collection("employee_database").
	...
	existingEmployees, err = client.Collection("employee_database").Documents(ctx).GetAll()
	if err != nil {
	...
	docRef := client.Collection("employee_database").Doc(newEmployeeID)
